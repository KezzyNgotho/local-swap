...
export default function CreateTrade() {
  ...
  // Mock: Suggest a rate based on recent trades, location, and time
  useEffect(() => {
    // In a real app, fetch recent trades, use geolocation, etc.
    // For demo, randomize a rate between 0.95 and 1.05 USD
    const randomRate = (0.95 + Math.random() * 0.1).toFixed(2);
    setSuggestedRate(randomRate);
    // Optionally, pre-fill currency based on location (mock: cUSD)
    // setSelectedCurrency(currencies.find(c => c.symbol === 'cUSD')!);
  }, [selectedCurrency]);
        ...
        <form id="create-trade-form" onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[#23262F] rounded-xl p-4 border border-[#333]">
            <div className="space-y-4">
              ...
              {/* Suggested Rate */}
              <div>
                <label className="block text-sm text-[#F0B90B] mb-1">Suggested Rate</label>
                <div className="flex items-center gap-2">
                  <span className="bg-[#181A20] text-[#F0B90B] px-3 py-1 rounded-lg font-mono text-base border border-[#F0B90B]">{suggestedRate} USD</span>
                  <button
                    type="button"
                    className="ml-2 px-3 py-1 bg-[#F0B90B] text-[#176B63] rounded-lg font-semibold hover:bg-[#e6a800] transition"
                    onClick={() => setPrice(suggestedRate)}
                  >
                    Use Suggested Rate
                  </button>
                </div>
