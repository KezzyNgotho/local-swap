...
const getSuggestedRate = (location, currency, hour) => {
  ...
  if (currency === "cUSD") {
    if (location.country === "Nigeria" || location.country === "US") return { rate: "1 cUSD ≈ 1500 NGN", currency: "NGN", reason: "Based on local market (Nigeria)" };
    if (location.country === "Ghana") return { rate: "1 cUSD ≈ 12 GHS", currency: "GHS", reason: "Based on local market (Ghana)" };
    if (location.country === "Kenya") return { rate: "1 cUSD ≈ 130 KES", currency: "KES", reason: "Based on local market (Kenya)" };
  }
  if (currency === "cEUR") {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          // Auto-select cStable based on country
          ...
          <form className="max-w-lg mx-auto bg-[#23262F] rounded-xl p-6 shadow-lg flex flex-col gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full rounded-lg px-3 py-2 bg-[#181A20] text-white border border-gray-700 focus:ring-2 focus:ring-[#F0B90B]"
                title="Select cStable coin"
              >
                {cStableOptions.map(opt => (
